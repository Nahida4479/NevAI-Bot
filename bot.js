...
client.on('messageCreate', async (message) => {
    debugging(`START processing message: ${message.content} at ${Date.now()}`); 
    ...
    response.content = response.content.replace(/\([^)]*\bResults?\b[^)]*\)/gi, '').replace(/\s{2,}/g, ' ').trim();
    ...
    if (response.imageResult && response.imageResult.length > 0) {
        ...
        debugging(`Exa image: ${attachment}`)
        } catch (err) {
